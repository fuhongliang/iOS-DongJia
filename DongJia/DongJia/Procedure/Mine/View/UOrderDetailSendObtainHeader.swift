//
//  UOrderDetailSendObtainHeader.swift
//  DongJia
//
...
//        $0.layer.masksToBounds = true
    ...
    /// 定位导航图标
    let carImage = UIImageView().then{
        $0.image = UIImage.init(named: "address_icon")
    }
    
    /// 地址
    let goodsWhere = UILabel().then{
        $0.textColor = .black
        $0.font = .systemFont(ofSize: 15)
    }
    ...
    /// 快递分割线
    let goodsWhereLine = UIView().then{
        $0.backgroundColor = .hex(hexString: "#F2F2F2")
    }
    ...
    override func configUI() {
        ...
        view.addSubview(goodsWhere)
        view.addSubview(carImage)
        view.addSubview(time)
        view.addSubview(goodsWhereLine)
        ...
        //MARK:快递状态
        goodsWhere.snp.makeConstraints { (make) in
            make.left.equalTo(carImage.snp.right).offset(10)
            make.top.equalTo(stateLine).offset(10)
        }
        //MARK:时间
        time.snp.makeConstraints { (make) in
            make.left.equalTo(goodsWhere)
            make.top.equalTo(goodsWhere.snp.bottom).offset(7)
        }
        
        //MARK:快递分割线
        goodsWhereLine.snp.makeConstraints { (make) in
            make.width.equalToSuperview().inset(15)
            make.centerX.equalToSuperview()
            make.top.equalTo(carImage.snp.bottom).offset(23)
            make.height.equalTo(1)
        }
        
        //MARK:地址图片
        image.snp.makeConstraints { (make) in
            make.top.equalTo(goodsWhereLine).offset(23)
            make.left.equalToSuperview().offset(15)
            make.size.equalTo(13)
        }
        //MARK:地址
        address.snp.makeConstraints { (make) in
            ...
            make.top.equalTo(goodsWhereLine).offset(10)
        }
        //MARK:收货人名字
